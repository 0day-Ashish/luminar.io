...
export default function ProofStep({
  ...
  return (
    <div className={`flex items-start space-x-4 p-5 rounded-2xl transition-all duration-300 ${currentStyles.container}`}>
      ...
      <div>
        <h4 className={`text-base font-instrument transition-all duration-300 ${currentStyles.title}`}>{title}</h4>
        <p className={`text-sm mt-1 font-instrument transition-all duration-300 ${currentStyles.description}`}>{description}</p>
      </div>
    </div>
  );
}
